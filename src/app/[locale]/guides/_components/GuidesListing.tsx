...
import { ChevronRight, ChevronLeft, MapPin, Clock, Sparkles, BookOpen, TrendingUp, Filter } from 'lucide-react';
import { getTranslations } from 'next-intl/server';
import { Link } from '@/i18n/navigation';
import { DESTINATIONS } from '@/config/destinations';
import { locales } from '@/i18n/config';
import type { ArticlePreview } from '../_lib/get-articles';

interface Props {
  locale: string;
  articles: ArticlePreview[];        // paginated subset to render
  totalArticles: number;             // full count across all pages
  currentPage: number;
  totalPages: number;
}

function buildPageHref(pageNum: number): string {
  return pageNum === 1 ? '/guides' : `/guides/page/${pageNum}`;
}

// Render a compact pager: 1, 2, 3 ... 28, 29, 30 (always include neighbours of current).
...
export default async function GuidesListing({
  locale,
  articles,
  totalArticles,
  currentPage,
  ...
  const totalGuidesAllLanguages = totalArticles * locales.length;
        ...
        <div className="container relative">
          ...
          <div className="max-w-3xl">
            ...
            <p className="text-base sm:text-lg md:text-xl text-ocean-50 leading-relaxed">
              {totalArticles > 0
                ? t('hero.subtitle', { count: totalGuidesAllLanguages, langCount: locales.length })
            ...
            {totalArticles > 0 && (
              <div className="grid grid-cols-3 gap-2 sm:flex sm:flex-wrap sm:items-center sm:gap-6 mt-6 md:mt-8">
                <div className="flex flex-col sm:flex-row items-center gap-1 sm:gap-2">
                  ...
                  <span className="font-semibold text-xs sm:text-sm text-center">{totalArticles} {t('hero.statGuides')}</span>
                </div>
                <div className="flex flex-col sm:flex-row items-center gap-1 sm:gap-2">
                  <div className="w-8 h-8 sm:w-10 sm:h-10 bg-white/15 backdrop-blur-sm rounded-xl flex items-center justify-center">
        ...
        <div className="container">
          {totalArticles > 0 ? (
            <>
              <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 md:mb-12">
                <div className="mb-4 md:mb-0">
                  ...
                  <p className="text-sm md:text-base text-slate-600">
                    {t('listing.subtitle', { count: totalArticles })}
                  </p>
                </div>
              ...
              {totalPages > 1 && (
                <nav
                  className="flex items-center justify-center gap-1 sm:gap-2 mt-12 md:mt-16"
                  aria-label="Pagination"
                >
                  {currentPage > 1 ? (
                    <Link
                      href={buildPageHref(currentPage - 1)}
                      className="flex items-center gap-1 px-3 py-2 rounded-lg border border-slate-200 hover:border-ocean-300 hover:bg-ocean-50 text-slate-700 font-medium transition-colors"
                      rel="prev"
                    >
                    ...
                    <span className="flex items-center gap-1 px-3 py-2 rounded-lg border border-slate-100 text-slate-300 font-medium cursor-not-allowed">
                      <ChevronLeft className="w-4 h-4" />
                      <span className="hidden sm:inline">{tCommon('previous')}</span>
                    </span>
                  )}
                  ...
                  {pageNumbers.map((p, i) =>
                    p === 'gap' ? (
                      ...
                      <span
                        key={p}
                        className="min-w-[2.5rem] px-3 py-2 rounded-lg bg-ocean-600 text-white font-bold text-center"
                        aria-current="page"
                      >
                        {p}
                      </span>
                      ...
                      <Link
                        ...
                        className="min-w-[2.5rem] px-3 py-2 rounded-lg border border-slate-200 hover:border-ocean-300 hover:bg-ocean-50 text-slate-700 font-medium text-center transition-colors"
                      >
                        {p}
                      </Link>
                    )
